...
- Vision model: `claude-opus-4-8` by default; `--model` also offers fable-5 (most capable, ~2.5× Opus cost) and sonnet-4-6 (cheapest/fastest) — Haiku excluded, no adaptive thinking. Streaming + adaptive thinking + structured outputs. Handle `stop_reason == "refusal"` before reading content.
